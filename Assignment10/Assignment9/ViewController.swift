...
//        generateDummyData()
    ...
    func fetchDataFromAPI() {
        APIUtils.shared.getAllItems() { items in
            self.items = items
        }
        
        APIUtils.shared.getAllLocations { locations in
            self.locations = locations
        }
        
        DatabaseManager.shared.saveRecords(items: items)
        DatabaseManager.shared.saveRecords(items: locations)
    }
